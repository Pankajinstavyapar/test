...
<head>
    ...
    <title>{{ str_replace('subdomain', $parent_locationData->state_name, $homepages->first()->pg_meta_title) }}</title>
    <meta name="keywords" content="{{ str_replace('subdomain', $parent_locationData->state_name, $homepages->first()->pg_meta_keyword) }}">
    <meta name="description" content="{{ str_replace('subdomain', $parent_locationData->state_name, $homepages->first()->pg_meta_desc) }}">
    ...
    @include('include.header-inner')
     ...
     <section class="pt-5">
        <div class="breadcrumb-area">
            <div class="container">
                <h1>{{ str_replace('subdomain', $parent_locationData->state_name, $homepages->first()->short_description) }}</h1>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('/') }}">Home</a></li>
                    <li class="breadcrumb-item active">{{ str_replace('subdomain', $parent_locationData->state_name, $homepages->first()->short_description) }}</li>
                </ul>
            </div>
        </div>
    </section>
    ...
    <section class="py-5 ">
        <div class="container">
            ...
            <div class="floated-abt">
                 @if ($item->image)
                        <img src="{{ url('images/' . $item->image) }}" alt="{{ str_replace('subdomain', $parent_locationData->state_name, $homepages->first()->short_description) }}">
                         @endif
                <!-- Share btn start -->
            ...
            <div class="section-title ">
                ...
                <h2 class="title">{{ str_replace('subdomain', $parent_locationData->state_name, $homepages->first()->short_description) }}</h2>
                @else
                     @endif
            </div>
            <p>@if ($homepages->isNotEmpty())
                        {!! htmlspecialchars_decode(str_replace('subdomain', $parent_locationData->state_name, $homepages->first()->description)) !!}
                    @else
                    @endif</p>
