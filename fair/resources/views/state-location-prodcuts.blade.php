...
<head>
  @php
    $originalmetatitle = $subdomainData->meta_title;
    $originaldiscription = $subdomainData->meta_description;
    $productTitle = $productData->product_title;
    $keyword = $subdomainData->meta_keywords;
    $location = $parent_locationData->state_name;
    $replacedmetatitle = str_replace(['keyword', 'subdomain'], [$productTitle, $location], $originalmetatitle);
    $replaceddiscription = str_replace(['keyword', 'subdomain'], [$productTitle, $location], $originaldiscription);
    $replacedkeyword = str_replace(['keyword', 'subdomain'], [$productTitle, $location], $keyword);
  @endphp
    
     @foreach($website as $web)
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{ $replacedmetatitle }}</title>
    <meta name="keywords" content="{{ $replacedkeyword }}">
    <meta name="description" content="{{ $replaceddiscription }}">
...
<body>
    ...
    @include('include.header-inner')
     ...
     <section class="pt-5">
         @php
    $originalShortDescription = $subdomainData->short_discription;
    $originalDescription = $subdomainData->description;
    $productTitle = $productData->product_title;
    $Description = $subdomainData->description;
    $location = $parent_locationData->state_name;
    $replacedDescription = str_replace(['keyword', 'subdomain'], [$productTitle, $location], $originalShortDescription);
    $replaceddescription = str_replace(['keyword', 'subdomain'], [$productTitle, $location], $originalDescription);
@endphp
        <div class="breadcrumb-area">
            <div class="container">
                <h1>{{ $replacedDescription }}</h1>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('/') }}">Home</a></li>
                    <li class="breadcrumb-item active">{{ $replacedDescription }}</li>
                </ul>
            </div>
        </div>
    </section>
    ...
    <section class="py-5 ">
        <div class="container">
            ...
            <div class="section-title ">
                ...
                <h2 class="title">{{ $replacedDescription }}</h2>
            </div>
            <p>{!! htmlspecialchars_decode($replaceddescription) !!}</p>
                    ...
                    <div class=" swiper-slide mb-4">
                        <div class="product-box">
                            ...
                            <div class="product-content">
                                <p class="pd-name mb-2"><a href="{{ url('/', ['state_url' => $parent_locationData->state_slug, 'product_url' => $data->product_url]) }}">{{ $data->product_title }}</a></p>
                                <p class="text-clamp1">{{ $data->sort_description }}</p>
                                <a href="{{ url('/', ['state_url' => $parent_locationData->state_slug, 'product_url' => $data->product_url]) }}" class="thm-btn  py-1">View Details</a>
                            </div>
                        </div>
                    </div>
                   @endforeach
