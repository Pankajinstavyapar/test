        ...
        <div class="page-header">
          <div class="page-block">
            <div class="row align-items-center">
              ...
              <div class="col-md-6">
                <div class="text-end">
                <a href="{{ route('AdminInsta.testimonial') }}" class="btn btn-primary">Add Testimonial</a>
            </div>
              </div>
            </div>
          </div>
                      ...
                      <tr>
                       ...
                       <td>
                        <div class="toggle-button">
                            <input type="checkbox" class="itemCheckbox visually-hidden" data-item-id="{{ $data->testimonial_id }}" data-initial-status="{{ $data->pg_status === 'active' ? 'active' : 'inactive' }}" {{ $data->pg_status === 'active' ? 'checked' : '' }}>
                            <label class="toggle-button-label" for="itemCheckbox"></label>
                        </div>
                    </td>
                        <td class="text-center">
                        <a href="{{ route('AdminInsta.testimonial-edit', $data->testimonial_id) }}" class="btn btn-primary btn-sm">Edit</a>
    
                    <form action="{{ route('AdminInsta.manage-testimonial.destroy', $data->testimonial_id) }}" method="post" style="display: inline-block">
                    @csrf
                    @method('DELETE')
    ...
    <script>
      $(document).ready(function () {
          $('.toggle-button').on('click', function () {
              ...
              $.ajax({
                  url: '{{ url("/AdminInsta/testi-status/") }}' + '/' + pgId + '/updateStatus',
                  method: 'POST',
                  data: {
                      _token: '{{ csrf_token() }}',
                      pg_status: pgStatus,
