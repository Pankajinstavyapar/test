        ...
        <div class="page-header">
          <div class="page-block">
            <div class="row align-items-center">
              ...
              <div class="col-md-6">
                <div class="text-end">
                <a href="{{ route('AdminInsta.pages') }}" class="btn btn-primary">Add Pages</a>
            </div>
              </div>
            </div>
          </div>
                ...
                <div class="table-responsive">
                  <table id="colum-render" class="table table-striped table-bordered nowrap">
                    <thead>
                      ...
                      <tbody>
                      @php
                      $pi = ($pages->currentPage() - 1) * $pages->perPage() + 1;
                      @endphp
                      ...
                      <tr>
                       ...
                       <td>
                        <div class="toggle-button">
                            <input type="checkbox" class="itemCheckbox visually-hidden" data-item-id="{{ $data->pg_id }}" data-initial-status="{{ $data->pg_status === 'active' ? 'active' : 'inactive' }}" {{ $data->pg_status === 'active' ? 'checked' : '' }}>
                            <label class="toggle-button-label" for="itemCheckbox"></label>
                        </div>
                    </td>
                        <td class="text-center">
                        <a href="{{ route('AdminInsta.pages-edit', $data->pg_id) }}" class="btn btn-primary btn-sm">Edit</a>
    
                    <form action="{{ route('AdminInsta.manage-pages.destroy', $data->pg_id) }}" method="post" style="display: inline-block">
                    @csrf
                    @method('DELETE')
      ...
      $(document).ready(function () {
          $('.toggle-button').on('click', function () {
              ...
              $.ajax({
                 ...
                 url: '{{ url("/AdminInsta/pg-status/") }}' + '/' + pgId + '/updateStatus',
                  method: 'POST',
                  data: {
                      _token: '{{ csrf_token() }}',
                      pg_status: pgStatus,
