...
<head>
   @php
    $originalkeyword = $subdomainData->meta_keywords;
    $productTitle = $productData->product_title;
    $originaldiscription = $subdomainData->meta_description;
    $originaltitle = $subdomainData->meta_title;
    $replacetitle = str_replace('keyword', $productTitle, $originaltitle);
    $replacekeyword = str_replace('keyword', $productTitle, $originalkeyword);
    $replacediscription = str_replace('keyword', $productTitle, $originaldiscription);
    @endphp
    ...
    <meta name="keywords" content="{{ $replacekeyword }}">
    <meta name="description" content="{{ $replacediscription }}">
...
<body>
    ...
    $replaceddescription = str_replace('keyword', $productTitle, $Description);
    $replacedDescription = str_replace('keyword', $productTitle, $originalShortDescription);
    @endphp
                    ...
                    <div class=" swiper-slide mb-4">
                        <div class="product-box">
                            ...
                            <div class="product-content">
                                <p class="pd-name mb-2"><a href="{{ url('/', ['subdomain_slug' => $subdomainData->subdomain_slug, 'product_url' => $data->product_url]) }}">{{ $data->product_title }}</a></p>
                                <p class="text-clamp1">{{ $data->sort_description }}</p>
                                <a href="{{ url('/', ['subdomain_slug' => $subdomainData->subdomain_slug, 'product_url' => $data->product_url]) }}" class="thm-btn  py-1">View Details</a>
                            </div>
                        </div>
                    </div>
                   @endforeach
